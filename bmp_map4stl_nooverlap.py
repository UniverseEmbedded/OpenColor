...
# Bitmap -> planar regions -> 4-color map -> 4 STL (A,B,C,D)
# Requirement: SAME POSITION, NO VOLUME OVERLAP.
# Fix: hole-preserving region polygon + boolean difference across 4 outputs.
...
def main():
    ap = argparse.ArgumentParser(description="Bitmap -> 4-color planar map -> 4 STL (A,B,C,D), same position, no overlap.")
    ap.add_argument("--in", dest="inp", required=True, help="Input bitmap (png/jpg/...)")
    ap.add_argument("--outdir", default="out_map4", help="Output directory")
    ap.add_argument("--name", default="map", help="Base name")
    ap.add_argument("--width-mm", type=float, default=80.0, help="Physical width in mm")
    ap.add_argument("--pixel-mm", type=float, default=0.35, help="Sampling pitch in mm")
    ap.add_argument("--thickness-mm", type=float, default=0.4, help="Extrusion thickness")
    ap.add_argument("--quant", type=int, default=16, help="Color quantization levels")
    ap.add_argument("--blur-sigma-px", type=float, default=0.0, help="Optional blur radius in pixels")
    ap.add_argument("--alpha-threshold", type=int, default=16, help="Alpha below this is background")
    ap.add_argument("--gap-mm", type=float, default=0.0, help="Inset regions (optional). 0 keeps exact tiling.")
    ap.add_argument("--simplify-mm", type=float, default=0.15, help="Polygon simplify tolerance")
    ap.add_argument("--min-area-mm2", type=float, default=1.0, help="Drop tiny regions")
    args = ap.parse_args()

    if not ensure_earcut():
        raise SystemExit("Missing triangulation engine mapbox_earcut. Add it to pixi and reinstall.")
    ...
    print(f"Raster {W} x {H} px, pixel {pixel_mm:.4f} mm, physical {args.width_mm:.2f} x {height_mm:.2f} mm")
    print(f"Regions: {nreg}")
    if nreg == 0:
        print("No printable regions.")
        return

    adj = build_adjacency(reg)
    rid_to_col = dsatur_4color(adj)

    # Build per-color polygon list (hole-preserving per region)
    ...
    # HARD GUARANTEE: no overlap volume (difference other colors)
    for c in range(4):
    ...
    for c in range(4):
        ...
        if mesh is None:
            print(f"{keys[c]}: empty")
            continue
        out = os.path.join(args.outdir, f"{args.name}_{keys[c]}.stl")
        mesh.export(out)
        print(f"{keys[c]}: wrote {out}  tris={len(mesh.faces)}")
