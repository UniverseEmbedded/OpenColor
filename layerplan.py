...
def solve_layers(
    ...
    # Target color in linear space
    r_lin, g_lin, b_lin, a_lin = rgba_to_linear_rgba(rgba)

    # Treat alpha as "coverage" composited over background.
    ...
    # Optional: reserve some transparent layers based on alpha.
    # This improves behavior when mode has 'T'.
    reserve_T = 0
    ...
    # If reserving T, solve the remaining composition without T, then add T back.
    ...
    # If T is the only material (e.g. user forbid everything else), fall back.
    if not mats_solve:
        mats_solve = mats
        tokens_solve = tokens
    ...
    # Enumerate integer compositions for the remaining layers.
