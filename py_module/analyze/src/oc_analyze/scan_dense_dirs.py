...
def scan_directory_for_dense_dirs(directory: Path, file_threshold: int) -> List[Dict[str, Any]]:
    """扫描目录，找出包含过多代码文件的子目录
    ...
    logger.info(f"在 {directory} 中找到 {len(results)} 个文件过多的目录（>{file_threshold}个文件）")
    return results


def generate_report(all_dense_dirs: List[Dict[str, Any]], scan_dirs: List[Path], file_threshold: int) -> str:
    """生成markdown报告
    ...
    for dir_name in sorted(dirs_by_root.keys()):
        ...
        for dir_info in dirs_sorted:
            # 格式化文件类型分布
            ext_dist = ", ".join([f"{ext}: {count}" for ext, count in sorted(dir_info['extensions'].items())])
            report_lines.append(
    ...
    for min_count, max_count, range_name in ranges:
        count = sum(1 for d in all_dense_dirs if min_count <= d['file_count'] <= max_count)
        if count > 0:
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="扫描指定目录中包含过多代码文件的子目录并生成报告")
    parser.add_argument(
        "scan_dirs",
        nargs="*",
...
def main():
    ...
    # 生成报告
    if all_dense_dirs:
        logger.info(f"总共找到 {len(all_dense_dirs)} 个文件数量超过 {args.threshold} 的目录")
    else:
