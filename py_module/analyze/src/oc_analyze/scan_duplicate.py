        ...
        for tok in tokenize.generate_tokens(io.StringIO(content).readline):
            ...
            if tok_type == tokenize.NAME:
                keywords = {'if', 'else', 'elif', 'for', 'while', 'def', 'class',
                          ...
                          'finally', 'with', 'yield', 'lambda', 'and', 'or',
                          'not', 'in', 'is', 'None', 'True', 'False', 'pass',
...
def find_duplicates_lsh(
    ...
    for group_blocks in similar_groups_dict.values():
        ...
        if len(unique_blocks) > 1:
            avg_similarity = sum(
                calculate_similarity(unique_blocks[0], b)
...
def generate_report(
    ...
    sorted_groups = sorted(
        groups,
        key=lambda g: (g.similarity_score, g.total_lines),
        reverse=True
    )
    ...
    for group in groups:
        for block in group.blocks:
            file_duplicates[block.file_path] = file_duplicates.get(block.file_path, 0) + 1
    ...
    sorted_files = sorted(file_duplicates.items(), key=lambda x: x[1], reverse=True)[:20]
...
def main():
    ...
    try:
        ...
        if duplicate_groups:
            ...
            similar_count = len([g for g in duplicate_groups if g.match_type == "similar"])
