...
def scan_directory_for_long_names(
    directory: Path, file_threshold: int, dir_threshold: int
):
    results = []
...
def scan_total_path_length(directory: Path, total_threshold: int = 72):
    ...
    dir_hits = len([r for r in results if r["type"] == "Directory"])
    logger.info(
        f"在 {directory} 中找到 {file_hits} 个长路径文件和 {dir_hits} 个长路径目录"
    )
    return results
...
def generate_report(
    ...
    if not all_hits:
        ...
        for dir_name in sorted(hits_by_dir.keys()):
            ...
            for info in hits_sorted:
                lines.append(
                    f"| {info['type']} | `{info['relative_path']}` | {info['name_len']} | {info['extension']} |"
                )
            lines.append("")
            lines.append("---")
            lines.append("")
    ...
    if not total_path_hits:
        ...
        for dir_name in sorted(total_hits_by_dir.keys()):
            ...
            for info in hits_sorted:
                lines.append(
                    f"| {info['type']} | `{info['rel_path']}` | {info['total_len']} | {info.get('extension', '')} |"
                )
            lines.append("")
            lines.append("---")
            lines.append("")
    ...
    for idx, info in enumerate(top, 1):
        scan_root = info.get("scan_root")
        display_path = (
            f"{scan_root.name}/{info['relative_path']}"
            if isinstance(scan_root, Path)
            else str(info["relative_path"])
        )
        lines.append(
            f"| {idx} | {info['type']} | `{display_path}` | {info['name_len']} |"
        )
    lines.append("")
    ...
    for idx, info in enumerate(top_total, 1):
        lines.append(
            f"| {idx} | {info['type']} | `{info['rel_path']}` | {info['total_len']} |"
        )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="扫描指定目录中文件名/目录名长度超限的代码文件并生成报告"
    )
    p.add_argument(
        "scan_dirs", nargs="*", help="要扫描的目录列表（不传则使用脚本内置默认目录）"
    )
    p.add_argument(
        "--threshold-file",
        type=int,
        default=24,
        help="文件名长度阈值（不含扩展名，默认24）",
    )
    p.add_argument(
        "--threshold-dir", type=int, default=24, help="目录名长度阈值（默认24）"
    )
    p.add_argument(
        "--threshold-total",
        type=int,
        default=72,
        help="总路径长度阈值（从项目根，默认72）",
    )
    p.add_argument(
        "--output-dir",
        type=str,
...
def main():
    ...
    if args.threshold_file <= 0 or args.threshold_dir <= 0 or args.threshold_total <= 0:
        raise ValueError(
            f"阈值必须大于0，当前: file={args.threshold_file}, dir={args.threshold_dir}, total={args.threshold_total}"
        )
    output_dir = Path(args.output_dir).expanduser().resolve()
    scan_dirs = (
        [Path(p).expanduser().resolve() for p in args.scan_dirs]
        if args.scan_dirs
    ...
    for d in scan_dirs:
        try:
            hits = scan_directory_for_long_names(
                d, args.threshold_file, args.threshold_dir
            )
