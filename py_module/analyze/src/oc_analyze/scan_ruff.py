...
def generate_report(
    ...
    for code in sorted_codes:
        ...
        sorted_issues = sorted(
            issues,
            key=lambda x: (str(x.get("path", "")), x.get("line", 0))
        )
...
def main():
    ...
    if not args.no_format_check:
        ...
        for scan_dir in scan_dirs:
            try:
                ...
                if format_issues:
                    logger.info(f"在 {scan_dir.name} 中找到 {len(format_issues)} 个格式问题")
                else:
    ...
    try:
        ...
        if all_issues:
            ...
            logger.info()
            logger.info("按规则类型统计：")
            for code in sorted(categorized.keys(), key=lambda x: len(categorized[x]), reverse=True):
