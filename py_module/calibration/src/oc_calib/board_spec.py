...
class BoardSpec:
    ...
    markers: Dict[str, Tuple[int, int]] = field(
        default_factory=lambda: {
            "TL": (0, 0),
            "TR": (33, 0),
            "BR": (33, 33),
...
def create_legacy_32x32_spec(
    color_system: str = "RYBW", n_layers: int = 5
) -> BoardSpec:
    ...
    spec = BoardSpec(
        ...
        cols=34,
        cell_size_mm=0.42,
        markers={"TL": (0, 0), "TR": (33, 0), "BR": (33, 33), "BL": (0, 33)},
    )
    ...
    for r in range(1, 33):
        for c in range(1, 33):
            ...
            spec.cell_map[f"{r},{c}"] = {"recipe_index": idx, "layers": recipe.layers}
