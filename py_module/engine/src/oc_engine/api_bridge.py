...
from oc_engine.handlers.board import handle_board_generate, handle_board_export_from_spec, \
...
def register_callback(callback: Callable[[str], None], log_callback: Optional[Callable[[str, str], None]] = None) -> None:
...
class _LogStream:
    def __init__(self, name: str, callback: Callable[[str, str], None], fallback) -> None:
        self.name = name
