...
def _get_resource_id(resource) -> int:
    """
    lib3mf resources have GetResourceID() and/or GetUniqueResourceID().
    The docs indicate GetResourceID exists and may be replaced by GetUniqueResourceID later.
    """
    ...
    *,
    ...
    try:
        if hasattr(model, "SetUnit") and hasattr(lib3mf, "ModelUnit"):
            unit = getattr(lib3mf.ModelUnit, "MilliMeter", None) or getattr(
                lib3mf.ModelUnit, "Millimeter", None
            )
            if unit is not None:
                model.SetUnit(unit)
    except Exception as e:
    ...
    # 使用 ColorGroup 替代 BaseMaterialGroup，这是正确的颜色分配方式
    color_group = model.AddColorGroup()
    color_group_rid = _get_resource_id(color_group)
    ...
    for slot in slot_names:
        ...
        if (
            tm is None
            or not isinstance(tm, trimesh.Trimesh)
            or tm.faces is None
            or len(tm.faces) == 0
        ):
            continue
        ...
        color_pid = int(color_property_ids.get(str(slot), 0))
...
def export_standard_3mf(
    *,
    ...
    model = ET.Element(
        f"{{{CORE_NS}}}model",
        {"unit": "millimeter", f"{{http://www.w3.org/2000/xmlns/}}m": MAT_NS},
    )
    ...
    for stl_path, slot in zip(stl_paths, slot_names):
        ...
        obj = ET.SubElement(
            resources,
            f"{{{CORE_NS}}}object",
            {"id": oid, "type": "model", "name": str(slot)},
        )
        # 创建网格元素
        ...
        tris = ET.SubElement(
            mesh,
            f"{{{CORE_NS}}}triangles",
            {"pid": bmg_id, "pindex": str(int(slot_index[slot]))},
        )
        for f in tm.faces:
            ET.SubElement(
                tris,
...
def export_standard_3mf_from_meshes(
    *,
    ...
    def _write_mesh_xml(fp, tm: trimesh.Trimesh, *, pindex: int) -> None:
        ...
        fp.write(
            f'<triangles pid="{int(bmg_id)}" pindex="{int(pindex)}">'.encode("utf-8")
        )
        m = int(f.shape[0])
    ...
    def write_to_zip(zf):
        ...
        with zf.open("3D/3dmodel.model", "w") as fp:
            ...
            fp.write(
                f'<model unit="millimeter" xmlns="{CORE_NS}" xmlns:m="{MAT_NS}">'.encode(
                    "utf-8"
                )
            )
            ...
            for idx, s in enumerate(slot_names):
                ...
                fp.write(
                    f'<m:base name="{name}" displaycolor="{displaycolor}"/>'.encode(
                        "utf-8"
                    )
                )
