...
def iter_svg_paths_with_group_transform(svg_path: str) -> Tuple[Dict[str, str], List[Tuple[Path, Dict[str, str], np.ndarray, bool, Optional[Tuple[int, int, int]]]]]:
...
def parse_transform_attr(s: str) -> np.ndarray:
    ...
    for fn, argstr in re.findall(r"([a-zA-Z]+)\s*\(\s*([^\)]*)\)", s):
        ...
        if fn == "matrix" and len(vals) == 6:
            ...
            if len(vals) >= 3:
                cx, cy = vals[1], vals[2]
                M = M @ _mat_translate(cx, cy) @ _mat_rotate(ang) @ _mat_translate(-cx, -cy)
            else:
                M = M @ _mat_rotate(ang)
    return M
