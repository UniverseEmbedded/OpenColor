...
def ui_apply_patch(lut: np.ndarray, coord, color: str, color_system: str):
    ...
    # 更新预览图
    lut_img = Image.fromarray(new_lut, mode="RGB").resize(
...
def ui_process_bitmap(
    ...
    if algo == "SDF (Smooth)":
        sp = SDFParams(
            smooth_sigma=float(smooth_sigma), simplify_eps=float(simplify_eps)
        )
        outputs = process_bitmap_sdf(
            image_path=image, lut_path=lut_file, params=p, sdf_params=sp, out_dir=out
        )
    else:
        outputs = process_bitmap(
            image_path=image, lut_path=lut_file, params=p, out_dir=out
        )
    ...
    return (
        str(outputs["preview_2d"]),
        str(outputs["preview_3d"]),
        str(zip_path),
        bambu_3mf,
    )
...
def build_bitmap_ui():
    ...
    with gr.Tab("Bitmap → Print Files"):
        with gr.Row():
            img_in = gr.Image(
                type="filepath", label="Bitmap (PNG/JPG, alpha supported)"
            )
            with gr.Column():
                ...
                algo = gr.Dropdown(
                    choices=["SDF (Smooth)", "Voxel (Legacy)"],
                    value="SDF (Smooth)",
                    label="Mesh Algorithm",
                )
                with gr.Row():
                    smooth_s = gr.Slider(
                        0.5, 3.0, value=1.2, step=0.1, label="SDF Smooth (sigma)"
                    )
                    simplify_e = gr.Slider(
                        0.0, 3.0, value=1.0, step=0.1, label="Simplify (eps)"
                    )

                alpha_t = gr.Slider(0, 255, value=10, step=1, label="Alpha threshold")
                auto_bg = gr.Checkbox(
                    value=False, label="Auto background remove (use top-left pixel)"
                )
