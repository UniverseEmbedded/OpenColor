...
def ui_generate_board(color_system: str, n_layers: int, cell_size_mm: float, layer_height_mm: float):
    """生成校准板STL文件和预览图"""
    params = BoardParams(color_system=color_system, n_layers=n_layers, cell_size_mm=cell_size_mm, layer_height_mm=layer_height_mm)
...
def build_board_ui():
    with gr.Tab("Calibration Board"):
        with gr.Row():
            cs = gr.Dropdown(choices=list(ALL_SYSTEMS.keys()), value="RYBW", label="Color System")
        ...
        btn.click(ui_generate_board, inputs=[cs, n_layers, cell, lh], outputs=[prev, zip_out])
