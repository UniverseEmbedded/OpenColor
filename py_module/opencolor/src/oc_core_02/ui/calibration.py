...
def ui_add_point(photo: np.ndarray, points: List[Tuple[float, float]], evt: gr.SelectData):
    """在照片上添加角点，并在图片上绘制标注"""
    if photo is None:
    ...
    for i, (px, py) in enumerate(pts):
        ...
        cv2.line(annotated_photo, (pt[0] - length, pt[1]), (pt[0] + length, pt[1]), color_main, 2)
        cv2.line(annotated_photo, (pt[0], pt[1] - length), (pt[0], pt[1] + length), color_main, 2)
        # 画个小圆圈中心
        cv2.circle(annotated_photo, pt, radius=3, color=(255, 255, 255), thickness=-1)
        
        # 标号 (1-based)
        cv2.putText(annotated_photo, str(i + 1), (pt[0] + 15, pt[1] - 15), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 3, cv2.LINE_AA)
        cv2.putText(annotated_photo, str(i + 1), (pt[0] + 15, pt[1] - 15), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, color_main, 1, cv2.LINE_AA)
...
def render_enhanced_preview(samples, board_spec, cell_measurements):
    ...
    img = Image.fromarray(samples).resize((out_w, out_h), resample=Image.Resampling.NEAREST).convert("RGBA")
        ...
        for c in range(w):
            ...
            if not is_data:
                ...
                if error > 5.0:
                    ...
                    draw.rectangle([x0, y0, x1, y1], outline=(255, 0, 0, alpha), width=3)

    return img

def ui_run_mcrt_analysis(obs, board_spec, all_samples, color_system_name, disable_cpp: bool):
    ...
    data_keys = [k for k, m in obs.cell_measurements.items() if m.get("is_data") and m.get("enabled")]
    if len(data_keys) == 0:
    ...
    if max_layer_index >= len(mat_props):
        return obs, None, f"材料索引超出范围：最大 {max_layer_index}，材料数量 {len(mat_props)}"

    for cell_idx, key in enumerate(data_keys):
        m = obs.cell_measurements[key]
        recipe = m.get("recipe")
        if not recipe or "layers" not in recipe: continue
        ...
        def _on_progress(done: int, total: int):
            ...
            progress(cell_base + ratio * cell_span, desc=f"MCRT 物理模拟中... {cell_index + 1}/{total_cells}")

        spec = mcrt_engine.simulate_layer_stack(
            mu_a_stack, mu_s_stack, g_stack, n_stack, heights,
            samples=500,
    ...
    # 3. 重新渲染
    new_preview = render_enhanced_preview(all_samples, board_spec, obs.cell_measurements)
    return obs, new_preview, f"MCRT 分析完成！平均误差: {avg_error:.2f}。已识别潜在矛盾点（红色光晕表示）。"
...
def ui_extract_lut(
    ...
    from oc_calib.calibration import estimate_coarse_homography, get_board_corners_from_h, perspective_warp_bgr, render_grid_overlay_bgr
    import json
    ...
    grid = render_grid_overlay_bgr(warped, total_rows=board_spec.rows, total_cols=board_spec.cols, line_step=1)

    # 采样所有单元格（包含边框）
    all_samples = extract_lut_from_warped_bgr(warped, total_rows=board_spec.rows, total_cols=board_spec.cols, window_px=int(window_px))
    ...
    for det in tag_debug_info.get("detections", []):
        ...
        cv2.putText(debug_overlay, f"ID:{det['id']}", (c[0][0], c[0][1]-10), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
    ...
    cv2.imwrite(str(debug_dir / f"debug_overlay_{photo_id}.png"), cv2.cvtColor(debug_overlay, cv2.COLOR_RGB2BGR))
...
def ui_probe_lut(all_samples, board_spec, obs, evt: gr.SelectData):
    ...
    # 更新预览图
    new_lut_img = render_enhanced_preview(all_samples, board_spec, obs.cell_measurements)
...
def build_calibration_ui(default_photo=None, default_spec=None):
    ...
    with gr.Tab("Extract Data from Photo"):
        ...
        with gr.Row():
            photo = gr.Image(type="numpy", label="Board photo (click corners)", value=default_photo)
            with gr.Column():
                with gr.Row():
                    spec_file = gr.File(label="Load BoardSpec (.json)", file_types=[".json"], value=default_spec)
                ...
                barrel = gr.Slider(-0.25, 0.25, value=0.0, step=0.001, label="Barrel (radial)")
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                mcrt_disable_cpp = gr.Checkbox(label="不用 C++ 后端（强制 Python）", value=False)
        ...
        load_spec_btn.click(ui_load_board_spec, inputs=[spec_file], outputs=[board_spec_state, spec_info])
        reset.click(ui_reset_points, inputs=[original_photo_state], outputs=[pts_state, pts_text, photo])
        auto_detect_btn.click(ui_auto_detect_points, inputs=[photo], outputs=[pts_state, pts_text, photo])
        
        photo.select(ui_add_point, inputs=[photo, pts_state], outputs=[pts_state, pts_text, photo])
        photo.upload(lambda x: (x, [], ""), inputs=[photo], outputs=[original_photo_state, pts_state, pts_text])
        ...
        extract.click(
            ...
            inputs=[photo, board_spec_state, pts_state, auto_detect, use_apriltag, zoom, barrel, offx, offy, win, auto_wb, vignette_fix],
            outputs=[warped, overlay, lut_prev, obs_state, all_samples_state],
        )

        lut_prev.select(ui_probe_lut, inputs=[all_samples_state, board_spec_state, obs_state], outputs=[obs_state, lut_prev, probe_html])
