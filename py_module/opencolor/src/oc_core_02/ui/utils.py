...
DEFAULT_OPTICAL_DATA = {
    "White":  {"mu_a": [0.10, 0.10, 0.10], "mu_s": [6.0, 6.0, 6.0], "g": 0.85, "n": 1.50},
    "Red":    {"mu_a": [0.35, 2.2, 2.6],   "mu_s": [3.0, 3.0, 3.0], "g": 0.85, "n": 1.50},
    "Yellow": {"mu_a": [0.45, 0.45, 2.4],  "mu_s": [3.0, 3.0, 3.0], "g": 0.85, "n": 1.50},
    ...
    "Magenta":{"mu_a": [0.50, 2.4, 0.50],  "mu_s": [3.0, 3.0, 3.0], "g": 0.85, "n": 1.50},
        ...
        for recipe in board_spec.cell_map.values():
            ...
            for i, idx in enumerate(layers):
                ...
                if idx not in idx_to_name:
                    ...
                    logger.error(f"[错误] BoardSpec 材料索引冲突：{idx} -> {idx_to_name[idx]} / {name}")
