...
from oc_proto.calib_color_rts.dataset_io import load_dataset, Dataset, filter_training_cells
from oc_proto.calib_color_rts.diagnostics import compute_diagnostics, render_boards_for_palette
from oc_xgb.xgb_fit import PhysGPRConfig, train_phys_gpr, predict_phys_gpr_lab, build_layer_sequences, build_gpr_features
...
class ODUnaryModel(BaseExtraModel):
    ...
    def fit(self, train_ds: Dataset, all_mats: list[str]):
        ...
        L = self.n_layers; M = len(self.mats)
        P = 1 + L * M
        N = len(seqs)
...
class PhysGPRModelWrapper(BaseExtraModel):
    ...
    def fit(self, train_ds: Dataset, all_mats: list[str]):
        ...
        y_lab = np.stack([rgb01_to_lab(c.measured_rgb.astype(np.float32) / 255.0) for c in train_cells])
        # 展平 y_lab 确保它是 (N, 3)
        if y_lab.ndim == 3:
    ...
    def predict(self, ds: Dataset):
        ...
        seqs = build_layer_sequences(recipes, self.material_keys, self.cfg.n_layers, layer_names_order=self.cfg.layer_names_order)
        
        # 提取基础 Lab (用于 GPR 特征)
        # 注意：这里需要 build_gpr_features，而它需要 base_lab
        from oc_xgb.xgb_fit import predict_ad_rgb01
        base_rgb01 = predict_ad_rgb01(
            seqs, self.material_keys, self.model.optical, 
        ...
        X, _ = build_gpr_features(
            recipes, self.material_keys, base_lab,
        ...
        pred_lab = predict_phys_gpr_lab(self.model, seqs, X, use_vulkan=self.cfg.use_vulkan, optical_model=self.cfg.optical_model)
...
def main():
    ...
    for name, model in models.items():
        ...
        try:
            ...
            for pid, ds in palettes.items():
                ...
                logger.info(f"  色盘 {pid}: Mean dE = {model_results[pid]['mean']:.4f}, P95 = {model_results[pid]['p95']:.4f}")
                ...
                if name == "RT-Stacking":
                    ...
                    render_boards_for_palette(diags, out_viz, palette_id=pid, flip_first_layer=True)
    ...
    # 3. 输出汇总表格
    logger.info("\n" + "="*80)
    logger.info(f"{'模型名称':<20} | {'A (Train)':<12} | {'B':<12} | {'C':<12} | {'D':<12} | {'E':<12}")
    logger.info("-" * 80)
