...
from oc_proto.calib_color_model_fit.dataset_io import load_dataset, apply_bw_calibration_inverse
...
from oc_proto.calib_color_model_fit.xgb_fit import (
    fit_optical_params, predict_ad_rgb01, PhysGPRConfig, OpticalParams, get_optical_model
)
...
def _generate_text_report(
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        sequences = build_layer_sequences(recipes, material_keys, n_layers=5, layer_names_order=layer_names_order)
        
        pred_rgb = predict_ad_rgb01(
            sequences, material_keys, optical_params, 
            use_vulkan=(impl_name == "vulkan"), 
            model_type=model_type
        )
        pred_lab = rgb01_to_lab(pred_rgb)
        
        real_rgb = np.stack([c.measured_rgb for c in fit_cells], axis=0).astype(np.float32) / 255.0
        ...
        for i, c in enumerate(fit_cells):
            ...
            f.write(f"| {c.row} | {c.col} | {recipe_str} | {r_u8} | {p_u8} | {de_errors[i]:.2f} |\n")
        ...
        f.write("| :--- | :--- |\n")
        pure_targets = ["White", "Black", "Red", "Green", "Blue", "Cyan", "Magenta", "Yellow"]
        pure_sequences = []
        pure_labels = []
        ...
        pure_rgb_test = predict_ad_rgb01(pure_sequences, material_keys, optical_params, use_vulkan=(impl_name == "vulkan"), model_type=model_type)
        ...
        sample_sequences = build_layer_sequences(sample_recipes, material_keys, n_layers=5, layer_names_order=layer_names_order)
        sample_pred_rgb = predict_ad_rgb01(sample_sequences, material_keys, optical_params, use_vulkan=(impl_name == "vulkan"), model_type=model_type)
        sample_real_rgb = np.stack([c.measured_rgb for c in sample_cells], axis=0).astype(np.float32) / 255.0
        ...
        for i, c in enumerate(sample_cells):
            ...
            label = "+".join(items)
            f.write(f"| {label} | {recipe_str} | {r_u8} | {p_u8} | {sample_de[i]:.2f} |\n")
...
def _rts_sequences_from_cells(cells: list, n_layers: int) -> list[list[str]]:
    seqs: list[list[str]] = []
    for c in cells:
        if isinstance(getattr(c, "layer_names", None), list) and len(getattr(c, "layer_names", []) or []) > 0:
...
def _rts_predict_rgb01_for_sequences(seqs: list[list[str]], rts_model: dict) -> np.ndarray:
...
def _generate_text_report_rts(
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("说明：RTS 直接拟合每种耗材的等效反射 r 与透射 t（按 RGB 三通道分别拟合），并用多次反射叠加公式做层叠。\n\n")
        ...
        real_rgb01 = np.stack([c.measured_rgb for c in fit_cells], axis=0).astype(np.float32) / 255.0
        ...
        sample_real = np.stack([c.measured_rgb for c in sample_cells], axis=0).astype(np.float32) / 255.0
...
def _run_single_visualization_rts(
    ...
    comp_img = _create_comparison_strip(list(sample_real_rgb), list(sample_pred_rgb), sample_labels, cell_w=150)
...
def _create_color_strip(colors_rgb: list[np.ndarray], labels: list[str], cell_w=200, cell_h=150) -> np.ndarray:
    n = len(colors_rgb)
    ...
    for i, (rgb, label) in enumerate(zip(colors_rgb, labels)):
        ...
        cv2.putText(canvas, f"{rgb_int.tolist()}", (x1 + 10, cell_h - 10), font, 0.4, t_color, 1)
        
    return canvas

def _create_comparison_strip(real_rgbs: list[np.ndarray], pred_rgbs: list[np.ndarray], labels: list[str], cell_w=200, cell_h=200) -> np.ndarray:
    n = len(real_rgbs)
    canvas = np.zeros((cell_h, n * cell_w, 3), dtype=np.uint8)
    font = cv2.FONT_HERSHEY_SIMPLEX
    ...
    for i, (real, pred, label) in enumerate(zip(real_rgbs, pred_rgbs, labels)):
        ...
        cv2.putText(canvas, "P:", (x1 + 5, cell_h//2 + 20), font, 0.4, t_color_pred, 1)
        
        # RGB values
        r_u8 = (real * 255).astype(int)
        p_u8 = (pred * 255).astype(int)
        cv2.putText(canvas, f"{r_u8.tolist()}", (x1 + 5, cell_h//2 - 10), font, 0.35, t_color_real, 1)
        cv2.putText(canvas, f"{p_u8.tolist()}", (x1 + 5, cell_h - 10), font, 0.35, t_color_pred, 1)
...
def _run_single_visualization(
    ...
    pure_rgb = predict_ad_rgb01(pure_sequences, material_keys, optical_params, use_vulkan=use_vulkan, model_type=model_type)
    pure_img = _create_color_strip(list(pure_rgb), pure_labels)
    pure_path = out_dir / f"pure_{model_type}_{impl_name}.png"
    cv2.imwrite(str(pure_path), pure_img)
    ...
    sample_pred_rgb = predict_ad_rgb01(sample_sequences, material_keys, optical_params, use_vulkan=use_vulkan, model_type=model_type)
    sample_real_rgb = np.stack([c.measured_rgb for c in sample_cells], axis=0).astype(np.float32) / 255.0
...
def main():
    ...
    # 加载数据进行拟合
    ds = load_dataset(palette_a_path, black_offset=args.black_offset, white_offset=args.white_offset)
    fit_cells = [c for c in ds.cells if c.enabled and c.recipe]
    recipes = []
    for c in fit_cells:
        r = dict(c.recipe)
        r["_layer_names"] = getattr(c, "layer_names", [])
        recipes.append(r)
    y_rgb01 = np.stack([c.measured_rgb for c in fit_cells], axis=0).astype(np.float32) / 255.0
    y_lab = rgb01_to_lab(y_rgb01)
    material_keys = sorted(list(set().union(*[r.keys() for r in recipes])))
    
    from oc_proto.calib_color_model_fit.xgb_features import build_layer_sequences
    sequences = build_layer_sequences(recipes, material_keys, n_layers=5, layer_names_order=str(args.layer_names_order))

    # 拟合配置
    cfg = PhysGPRConfig(
        n_layers=5, opt_steps=300, opt_reg=1e-6,
        gpr_noise=1e-7, gpr_lengthscale=0.1, gpr_signal=1.0, gpr_jitter=1e-9,
