...
class PhysGPRModelImpl(ColorPredictionModel):
    ...
    def __init__(
        ...
        # 初始化预测模型
        self._optical_model = VulkanFourFluxModel() if use_vulkan else NumpyFourFluxModel()
    ...
    def _predict_physical(self, sequences: list[list[str]]) -> np.ndarray:
        ...
        raw_params = RawOpticalParams(
            mu_a=self.optical.mu_a,
            mu_s=self.optical.mu_s,
            g=self.optical.g
        )
    ...
    def _sequences_to_recipes(self, sequences: list[list[str]]) -> list[dict[str, float]]:
        """将层序列转换为配方"""
        recipes = []
