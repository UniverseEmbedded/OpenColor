...
from oc_xgb.xgb_features import (
    infer_material_keys,
    build_gpr_features,
...
def _load_all_datasets(
    dataset_path: Path, black_offset: float = 0.0, white_offset: float = 0.0
) -> dict[str, Dataset]:
    """
    加载所有色盘的数据集。
...
def synthesize_palettes_from_a(
    ...
    train_rgb01 = (
        np.stack([c.measured_rgb for c in train_cells], axis=0).astype(np.float32)
        / 255.0
    )
    ...
    for pid, ds in datasets.items():
        ...
        msg = (
            "生成合成对照(target_rgb)"
            if should_force and (not needs_syn)
            else ("自动补充合成(measured_rgb)" if needs_syn else "跳过")
        )
        logger.info(
            f"[{VERSION}] 已用色盘{train_ds.palette_id}的物理参数 {msg}: 色盘 {ds.palette_id}"
        )
...
def run_fit(args: FitArgs) -> None:
    ...
    if args.black_offset > 0 or args.white_offset > 0:
        logger.info(
            f"[{VERSION}] 黑白校准已启用: black_offset={args.black_offset}, white_offset={args.white_offset}"
        )
    else:
        logger.info(f"[{VERSION}] 黑白校准已禁用")

    logger.info(
        f"[{VERSION}] 加载了 {len(datasets)} 个色盘的数据: {list(datasets.keys())}"
    )
    ...
    if train_pid not in datasets:
        raise RuntimeError(
            f"未找到色盘A的数据集（dataset_cells_A.json / dataset_cells.json）。当前已有: {list(datasets.keys())}"
        )
    # 为 B~E 生成“外推合成对照值”写入 target_rgb（不覆盖其真实 measured_rgb）
    # 同时对缺失测量值的色盘（例如 F~H）自动写入 measured_rgb
    synthesize_palettes_from_a(
        datasets, train_pid, cfg, force_synthesis_ids=["B", "C", "D", "E"]
    )
    ...
    logger.info(
        f"[{VERSION}] 色盘 A: {len(train_cells_a)} 个训练样本 (仅此色盘参与训练)"
    )
    ...
    rgb01_train = (
        np.stack([c.measured_rgb for c in all_train_cells], axis=0).astype(np.float32)
        / 255.0
    )
    ...
    model = train_phys_gpr(
        train_recipes, Y_train, material_keys, cfg, metadata=meta_list
    )
    ...
    for pid, ds in sorted(datasets.items()):
        ...
        pred_u8, X_all, bias_round = _predict_for_dataset(
            ds, model, cfg, material_keys, pid_memorizer
        )
    ...
    if memorize_enabled:
        logger.info(
            f"[{VERSION}] memorizer: 覆盖 {total_override}/{len(all_train_cells)} 个训练格子"
        )
            ...
            json.dumps(
                {
                    "version": VERSION,
                    "datasets": {
                        pid: str(ds.palette_id) for pid, ds in datasets.items()
                    },
