...
def perspective_warp_bgr(img_bgr, pts, wp, inset_mode=False, rows=1, cols=1):
    ...
    if inset_mode:
        ...
        dst_pts = np.array(
            [[0, 0], [wp.dst_size, 0], [wp.dst_size, wp.dst_size], [0, wp.dst_size]],
            dtype=np.float32,
        )
...
class AppState:
    def __init__(self):
        ...
        self.photos_dir = Path(
            r"D:\pama1234\pfp\p-2026-01\OpenColor-02\data\calibration\photos_02"
        )
    ...
    def _ensure_specific_spec(self):
        ...
        if gen_out_dir.exists():
            for spec_path in gen_out_dir.glob("*_board_spec.json"):
                ...
                if (
                    not dest.exists()
                    or spec_path.stat().st_mtime > dest.stat().st_mtime
                ):
...
def rebuild_warped_from_json(
    warp_json_path: Path, photo_path: Path, out_dir: Path
) -> None:
    ...
    warped_bgr = perspective_warp_bgr(
        img_bgr, pts, wp, inset_mode=True, rows=rows, cols=cols
    )
    ...
    write_manifest(
        output_dir=out_dir,
        version=VERSION,
        inputs=[str(photo_path), str(warp_json_path)]
...
def load_spec(spec_name):
    ...
    return (
        f"已加载: {spec.name} ({spec.rows}x{spec.cols})",
        preview,
        str(state.current_out_dir),
    )
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if cell_raw:
                ...
                cv2.rectangle(
                    img, (x0, y0), (x0 + cell_size, y0 + cell_size), color, -1
                )
                cv2.rectangle(
                    img, (x0, y0), (x0 + cell_size, y0 + cell_size), (100, 100, 100), 1
                )
...
def auto_detect_tags(img):
    ...
    if H is not None:
        ...
        return (
            img,
            "自动检测失败（已尝试 AprilTag 和 Chroma），请手动点击。",
            gr.update(),
        )
...
def generate_warped(img, inset_mode):
    ...
    overlay_bgr = render_grid_overlay(
        warped_bgr, state.current_spec.rows, state.current_spec.cols
    )
    ...
    return (
        Image.fromarray(cv2.cvtColor(warped_bgr, cv2.COLOR_BGR2RGB)),
...
with gr.Blocks() as demo:
        ...
        with gr.Column():
            with gr.Group():
                ...
                spec_dropdown = gr.Dropdown(
                    choices=spec_files,
                    value=default_spec
                    if default_spec in spec_files
                    else (spec_files[0] if spec_files else None),
                    label="选择 Board Spec",
                )

                photo_files = [
                    f.name
                    for f in state.photos_dir.glob("*")
                    if f.suffix.lower() in [".jpg", ".jpeg", ".png", ".bmp"]
                ]
                photo_dropdown = gr.Dropdown(
                    choices=photo_files, label="从 photos_02 目录选择照片"
                )
            ...
            input_img = gr.Image(
                value=str(state.default_photo_path)
                if state.default_photo_path and state.default_photo_path.exists()
                else None,
            ...
            with gr.Row():
                inset_checkbox = gr.Checkbox(
                    label="手动点击模式：缩进一格", value=False
                )
    ...
    spec_dropdown.change(
        load_spec,
        inputs=[spec_dropdown],
        outputs=[spec_info, spec_preview, out_dir_input],
    )
    out_dir_input.change(
        handle_out_dir_change, inputs=[out_dir_input], outputs=[status_text]
    )
    photo_dropdown.change(
        handle_photo_select, inputs=[photo_dropdown], outputs=[input_img, status_text]
    )
    input_img.upload(
        handle_image_upload, inputs=[input_img], outputs=[input_img, status_text]
    )
    input_img.select(handle_click, inputs=[input_img], outputs=[input_img, status_text])
    detect_btn.click(
        auto_detect_tags,
        inputs=[input_img],
        outputs=[input_img, status_text, inset_checkbox],
    )
    warp_btn.click(
        generate_warped,
        inputs=[input_img, inset_checkbox],
        outputs=[warped_out, overlay_out, result_info],
    )
...
if __name__ == "__main__":
    ...
    if args.rebuild:
        warp_json = (
            Path(args.warp_json)
            if args.warp_json
            else state.current_out_dir / "warp.json"
        )
