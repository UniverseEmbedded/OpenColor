...
def _perspective_warp_bgr(img_bgr: np.ndarray, pts, dst_size: int, *, inset_mode: bool, rows: int, cols: int) -> np.ndarray:
    if inset_mode:
...
def _resolve_warped_image_path(board_dir: Path, photos_root: Path, cache_dir: Path, *, rows: int, cols: int) -> Path | None:
    ...
    if point_order != "TL_TR_BR_BL":
        logger.warning(f"[{VERSION}] 警告: 暂未实现 point_order={point_order}，将按 TL_TR_BR_BL 处理: {warp_json}")
    ...
    if isinstance(inset_mode, bool):
        ...
        if rows == 17 and cols == 17:
            ...
            logger.info(f"[{VERSION}] 自动判定 inset_mode={use_inset} (rows={rows}, cols={cols})")
        else:
            use_inset = False
    ...
    logger.info(f"[{VERSION}] 已从 {raw_img.name}+warp.json 生成输入: {out_warped} (inset_mode={use_inset}, rotation_count={rotation_count})")
    return out_warped
...
def _resolve_board_spec_path(board_dir: Path) -> Path | None:
    ...
    if manifest_path.exists():
        try:
            ...
            logger.error(f"[{VERSION}] 警告: 读取 manifest.json 失败，将回退到按目录名推断 spec: {manifest_path} ({e})")
    ...
    if not src.exists():
        logger.info(f"[{VERSION}] 未找到规格文件: {src}，正在尝试通过 calib_board_gen 生成...")
        try:
...
def run(warped_img_path: Path, spec_path: Path, patched_json_path: Path, out_dir: Path, ref_white=None, ref_black=None):
    ...
    if patched_json_path.exists():
        patched = json.loads(patched_json_path.read_text(encoding="utf-8", errors="replace"))
    else:
        # Generate default observations if patched file is missing
        # 修正：生成逻辑网格数据 (通常是 15x15) 而不是物理网格数据
        logger.info(f"提示: 未找到 patched 文件 {patched_json_path}，将按 spec 生成默认逻辑网格数据。")
        patched = []
    ...
    if rows == 17:
        ...
        if max_r_patched < 16 and max_c_patched < 16:
            row_offset = 1
            col_offset = 1
            logger.info(f"[{VERSION}] 检测到 15x15 数据在 17x17 网格中，应用居中偏移 (+1)")
    ...
    for obs in patched:
        ...
        if cell_raw:
            slot_palette_names, layer_indices, layer_names = extract_layer_sequence(cell_raw)
        ...
        if x1 > x0 and y1 > y0:
            cv2.rectangle(recipe_sum_board, (x0, y0), (x1, y1), min(255, int(recipe_sum * 10)), -1)
            ...
            if layer_names:
                cv2.putText(first_layer_vis, layer_names[0][:1], (x0 + 2, y0 + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (128, 128, 128), 1)
        ...
        if sx1 > sx0 and sy1 > sy0:
            ...
            # 添加行列文字标注
            label = f"{lr},{lc}"
            cv2.putText(vis, label, (sx0 + 2, sy0 + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.3, color, 1)
    ...
    if white_probe is not None and black_probe is not None:
        logger.info(f"[{VERSION}] 校准前探针格: 白色候选(R{white_probe['row']}C{white_probe['col']}, white_w={max_white_probe})"
            f" RGB={white_probe['measured_rgb']} sample_box={white_probe.get('sample_box_px')}"
        )
        logger.info(f"[{VERSION}] 校准前探针格: 黑色候选(R{black_probe['row']}C{black_probe['col']}, black_w={max_black_probe})"
    ...
    if darkest is not None and brightest is not None:
        dc = darkest[1]
        bc = brightest[1]
        logger.info(f"[{VERSION}] 校准前亮度最黑: R{dc['row']}C{dc['col']} RGB={dc['measured_rgb']} recipe={dc.get('recipe')} sample_box={dc.get('sample_box_px')}"
        )
        logger.info(f"[{VERSION}] 校准前亮度最白: R{bc['row']}C{bc['col']} RGB={bc['measured_rgb']} recipe={bc.get('recipe')} sample_box={bc.get('sample_box_px')}"
        )
    ...
    if ref_white is not None and ref_black is not None:
        ...
        if white_ref_cell is not None and black_ref_cell is not None:
            ...
            logger.info(f"[{VERSION}] 自动寻找参考点: 白色(R{white_ref_cell['row']}C{white_ref_cell['col']})={rgb_w}, 黑色(R{black_ref_cell['row']}C{black_ref_cell['col']})={rgb_b}")
    ...
    def check_duplicates(cells):
        ...
        for i, c in enumerate(cells):
            if not c["enabled"] or not c["has_recipe"]:
                continue
            feat = (tuple(c.get("layer_names") or []), tuple(sorted(c.get("recipe", {}).items())))
            seen[feat].append(c)
        ...
        for feat, group in seen.items():
            if len(group) > 1:
                ...
                logger.warning(f"[{VERSION}] 警告: 发现物理重复格子! 数量={len(group)}, 位置={locs}, 特征={feat}")
        
        if duplicates > 0:
            logger.info(f"[{VERSION}] 统计: 共发现 {duplicates} 个格子存在特征冲突（物理属性完全相同）")
        else:
    ...
    manifest = {
        ...
        "preview_after": out_preview_after.name if (rgb_w is not None and rgb_b is not None) else None,
...
def main():
    ...
    ap.add_argument("--photos-root", type=str, 
    ...
    ap.add_argument("--patched", type=str, default="", help="Path to observation_patched.json")
    args = ap.parse_args()
    ...
    if board_a_dir.is_dir():
        ...
        if spec and spec.exists():
            ...
            warped = _resolve_warped_image_path(board_a_dir, photos_root, input_cache_dir, rows=spec_shape.rows, cols=spec_shape.cols)
        ...
        if warped and spec and warped.exists() and spec.exists():
            try:
                ...
                logger.info(f"  Board_A 处理成功，获取到全局参考: 白色={global_white}, 黑色={global_black}")
            except Exception as e:
    ...
    for board_dir in photos_root.iterdir():
        ...
        if spec and spec.exists():
            spec_shape = SpecAdapter(spec)
            warped = _resolve_warped_image_path(board_dir, photos_root, input_cache_dir, rows=spec_shape.rows, cols=spec_shape.cols)
        patched = board_dir / "observation_patched.json"
        
        if not warped or not spec or not warped.exists() or not spec.exists():
            logger.info(f"  跳过: 缺少必要文件 (warped={bool(warped and warped.exists())}, spec={bool(spec and spec.exists())})")
            continue
        ...
        try:
            run(warped, spec, patched, board_out, ref_white=global_white, ref_black=global_black)
            processed_count += 1
