...
def rebuild_stl_to_2d(stl_path: Path, layer_z_mm: float, board_mm: float, pixel_size: int) -> tuple:
    """
...
def main():
    ...
    for slot_name, layer in problem_layers:
        ...
        if report_path.exists():
            try:
                ...
                for detail in report.get("details", []):
                    if detail.get("slot_name") == slot_name and detail.get("layer") == layer:
        ...
        evidence = {
            ...
            "area_diff_percent": round(abs(stl_rebuild_area - poly_area) / poly_area * 100, 4) if poly_area > 0 else 0,
        }
    ...
    for r in results:
        ...
        if area_diff > 2.0:
            ...
            logger.info(f"  → [情况2] 对比图生成有bug (重建面积≈SVG, 但compare面积不同)")
        elif missing > 0 and extra > 0 and abs(missing - extra) / (missing + extra) < 0.3:
