...
def stl_to_bitmap(stl_path: Path, layer_z_mm: float, pixel_w: int, pixel_h: int,
                  board_mm: float) -> np.ndarray:
    """
    将STL文件的指定层转换为位图
    """
...
def extract_slot_mask_from_preview(preview_image: np.ndarray, slot_rgb: tuple,
    ...
    for i in range(3):
        mask += (np.abs(preview_image[:, :, i].astype(np.float32) - slot_rgb[i]) < color_tolerance).astype(np.float32)
    
    # 至少2个通道匹配
    return (mask >= 2).astype(np.float32)


def compare_bitmaps(bitmap_stl: np.ndarray, bitmap_poly: np.ndarray,
                    slot_name: str, layer: int) -> dict:
    """
    对比两个位图，返回统计信息
    """
...
def run_stl_poly_comparison(export_dir: Path, poly_dir: Path, manifest: dict,
                            stl_files: list, slot_names: list, slot_preview_rgb: dict,
                            compare_root_dir: Path = None) -> dict:
    """
    运行STL与多边形预览图的对比
    ...
    # 创建对比输出目录（强制清理旧产物）
    compare_dir = compare_root_dir if compare_root_dir else (export_dir / "07_stl_poly_compare")
    ...
    for stl_path in stl_files:
        ...
        for layer in range(n_layers):
            ...
            # 读取单色栅格图并归一化到0-1
            poly_mask = np.array(Image.open(raster_path).convert("L"), dtype=np.float32) / 255.0
                    ...
                    if polys:
                        ...
                        if merged_poly.is_empty:
                            ...
                            if poly_mask_from_svg is None:
                                ...
                                # 对比SVG truth
                                stats_svg = compare_bitmaps(stl_bitmap, poly_mask_from_svg, slot_name, layer)
                                # 如果差异显著不同，打印警告
                                if abs(stats_svg['diff_ratio'] - stats['diff_ratio']) > 0.01:
                                    logger.warning(f"    [双truth警告] raster差异={stats['diff_ratio']*100:.2f}%, "
            ...
            # 打印所有层的对比结果（不管差异大小）
            logger.info(f"  {slot_name} L{layer}: 差异 {stats['diff_ratio']*100:.2f}% "
    ...
    logger.info(f"\nSTL与多边形对比完成:")
    logger.info(f"  总体差异: {total_diff_pixels}/{total_pixels} 像素 ({overall_diff_ratio*100:.2f}%)")
