...
def predict_fourflux(
    data: np.ndarray, recipe: list[int], n_layers: int = 5
) -> np.ndarray:
    """使用FourFlux模型预测颜色"""
...
def diagnose_pure_color(
    data: np.ndarray, meta: dict, color_name: str, rgb_values: list[float]
):
    """诊断单个纯色"""
    logger.info(f"\n{'=' * 60}")
    ...
    logger.info(
        f"目标Lab: [{float(target_lab[0]):.1f}, {float(target_lab[1]):.1f}, {float(target_lab[2]):.1f}]"
    )
    ...
    if color_upper in material_keys:
        ...
        logger.info(
            f"  预测Lab: [{float(pred_lab[0]):.1f}, {float(pred_lab[1]):.1f}, {float(pred_lab[2]):.1f}]"
        )
