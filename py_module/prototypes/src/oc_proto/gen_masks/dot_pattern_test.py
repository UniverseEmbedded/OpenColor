...
def _draw_dot_cell(
    ...
    if hue_range is None:
        ...
        return _draw_gray_dots(
            draw, draw_x1, draw_y1, draw_x2, draw_y2,
            dot_size, row_idx, col_idx
        )
...
def _draw_overlap_cell(
    ...
    # 绘制波点A (底层/左侧)
    draw.ellipse(
        [center_x_a - radius, center_y - radius,
         center_x_a + radius, center_y + radius],
        fill=color_a + (255,)
    )
    ...
    # 绘制波点B (顶层/右侧，带偏移)
    draw.ellipse(
        [center_x_b - radius, center_y - radius,
         center_x_b + radius, center_y + radius],
        fill=color_b + (255,)
    )
    ...
    label_w = 28
    label_h = 14
    draw.rectangle([x1+2, y1+2, x1+2+label_w, y1+2+label_h], fill=(255, 255, 255, 200))
...
def run_gen_masks_pipeline(
    ...
    with open(preview_dir / "01_error_stats_predicted.json", "w", encoding="utf-8") as f:
        json.dump(error_stats, f, indent=2, ensure_ascii=False)
    logger.error(f"误差统计: 平均DeltaE={error_stats['mean_deltaE']:.2f}, 最大={error_stats['max_deltaE']:.2f}")
    
    # 生成每层调色板预览图
    rgb_lut = np.asarray([cs.slot_preview_rgb[n] for n in cs.slot_names], dtype=np.uint8)
    logger.info("生成每层调色板预览图...")
    for z in range(n_layers):
        digits_z = recipe_digits[idxs, int(z)].reshape(h, w).astype(np.int32, copy=False)
    ...
    ys = ys.reshape(-1)
    xs = xs.reshape(-1)
    volumes = generate_layer_volumes(params, cs, ys, xs, idxs, h, w, recipe_digits=recipe_digits)
