...
该文件承载 main.py 中的主要实现逻辑，避免入口文件过于臃肿。
"""
...
def _save_layer_total_contour_viz_full(
    ...
    for z in range(n_layers):
        ...
        contours_union, _ = cv2.findContours(
            union_u8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        contours_gap, _ = cv2.findContours(
            gap_u8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        contours_overlap, _ = cv2.findContours(
            overlap_u8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        logger.info(
            f"  L{z:02d}: 已保存 {out_path.name} (覆盖={int(np.count_nonzero(covered))}, 空缺={int(np.count_nonzero(gap))}, 重叠={int(np.count_nonzero(overlap))})"
        )
...
def _analyze_solved_palette_vs_solver_input(
    ...
    *,
    ...
    if tgt_u8.shape[0] != preview_out_h or tgt_u8.shape[1] != preview_out_w:
        tgt_u8 = np.array(
            Image.fromarray(tgt_u8).resize(
                (preview_out_w, preview_out_h), resample=Image.Resampling.BILINEAR
            ),
            dtype=np.uint8,
        )
    if mask.shape[0] != preview_out_h or mask.shape[1] != preview_out_w:
        mask_u8_r = np.array(
            Image.fromarray(mask_u8).resize(
                (preview_out_w, preview_out_h), resample=Image.Resampling.NEAREST
            ),
            dtype=np.uint8,
        )
        mask = mask_u8_r > 127
    ...
    logger.info(
        f"开始计算 solved_palette vs 求解输入(03_preprocessed_for_solver) 的偏差: {out_an_dir}"
    )
    for z in range(int(n_layers)):
        ...
        logger.info(
            f"[偏差统计] L{z:02d}_solved_palette vs 03_preprocessed_for_solver ΔE76:"
        )
        ...
        de_mix = delta_e_cie76(
            rgb01_to_lab(tgt_blur_rgb01), rgb01_to_lab(sp_blur_rgb01)
        )
        stats_mix = {
        ...
        logger.info(
            f"[混色偏差统计] L{z:02d}_solved_palette vs 03_preprocessed_for_solver (眯眼高斯sigma={mix_sigma:.2f}) ΔE76:"
        )
        ...
        heat_clip_mix = (
            float(stats_mix["P95ΔE76"]) if float(stats_mix["P95ΔE76"]) > 1e-6 else 1.0
        )
        ...
        with open(
            out_an_dir / f"L{z:02d}_de76_mix_{mix_tag}_stats.json",
            "w",
            encoding="utf-8",
        ) as f:
        ...
        logger.info(
            f"  L{z:02d}: 已保存 {heat_path.name}、L{z:02d}_de76_stats.json、"
...
def run(
    ...
    *,
    ...
    volumes_raw = _generate_layer_volumes(
        params, cs, ys, xs, inverse, h, w, recipe_digits=recipe_digits
    )

    if bool(joint_l0_enabled) and int(n_layers) > 0:
        logger.info(
            f"[优化] 首层联合优化: enabled=True, passes={int(joint_l0_passes)}, use_icm={bool(joint_l0_use_icm)}"
        )

        guidance_gray = cv2.cvtColor(rgb_u8, cv2.COLOR_RGB2GRAY).astype(np.float32) / 255.0

        pix_rgb01_foreground = rgb01[ys, xs]
        recipes_per_pixel = recipe_digits[inverse]
        ...
        try:
            ...
            logger.info(
                f"[信息] 唯一颜色数: {len(recipe_digits)}, 像素数: {len(recipes_per_pixel)}"
            )
            ...
            if rp.ndim != 2 or inv.ndim != 1 or int(rp.shape[0]) != int(inv.shape[0]):
                raise RuntimeError(
                    f"配方映射输入形状异常: recipes_per_pixel={rp.shape}, inverse={inv.shape}"
                )
            ...
            for z in range(int(rp.shape[1])):
                ...
                cpu_recipe_digits[:, z] = np.argmax(counts, axis=1).astype(
                    np.int32, copy=False
                )
            ...
            try:
                ...
                if gpu_recipe_digits.shape != cpu_recipe_digits.shape:
                    raise RuntimeError(
                        f"GPU配方映射输出形状异常: {gpu_recipe_digits.shape} != {cpu_recipe_digits.shape}"
                    )
                ...
                if sample_n > 0:
                    ...
                    sample_idx = (
                        rng.choice(n_colors, size=sample_n, replace=False)
                        if n_colors > sample_n
                        else np.arange(n_colors)
                    )
                    mismatch = int(
                        np.count_nonzero(
                            gpu_recipe_digits[sample_idx] != cpu_recipe_digits[sample_idx]
                        )
                    )
                    mismatch_ratio = float(mismatch) / float(
                        sample_idx.size * cpu_recipe_digits.shape[1]
                    )
                    if mismatch_ratio > 0.001:
                        raise RuntimeError(
                            f"GPU配方映射疑似错误: mismatch_ratio={mismatch_ratio:.6f}"
                        )
            ...
            for z in range(n_layers):
                ...
                Image.fromarray(layer_img).save(
                    layer_viz_joint_dir / f"L{z:02d}_solved_palette.png"
                )
    ...
    for z in range(n_layers):
        ...
        _save_layer_total_contour_viz(
            ...
            z,
            layer_total,
            str(layer_total_dir / f"L{z:02d}_bitmap_total_overlap_gap.png"),
        )
