...
from oc_proto.gen_masks.island_suppress import (
...
def _solve_and_optimize(
    ...
    if int(len(ys)) == 0:
        raise RuntimeError(
            "前景像素为空：请检查输入图是否全透明，或背景移除/阈值设置是否错误"
        )
    ...
    logger.info(
        f"[信息] 待求解像素数={int(pix.shape[0])}，唯一颜色数={int(unique_pix.shape[0])}"
    )
    ...
    if cache_path.exists():
        try:
            ...
            n_layers0 = int(
                data.get("n_layers", rec0.shape[1] if rec0.ndim == 2 else -1)
            )
            if (
                up0.shape == unique_pix.shape
                and rec0.ndim == 2
                and int(rec0.shape[0]) == int(unique_pix.shape[0])
                and int(rec0.shape[1]) == int(n_layers)
                and int(n_layers0) == int(n_layers)
            ):
                if bool(
                    np.allclose(up0, unique_pix.astype(np.float32), atol=1e-7, rtol=0.0)
                ):
    ...
    if bool(first_print_layer_bias_enabled):
        logger.info(
            f"[优化] 首层贴近原图优化: enabled=True, slack_de76={float(first_print_layer_bias_slack_de76):.3f}"
        )
...
def _postprocess_masks(
    ...
    if postprocess_mode != "none":
        ...
        if postprocess_mode in {"guided", "joint"}:
            ...
            guidance_gray = (
                cv2.cvtColor(rgb_u8, cv2.COLOR_RGB2GRAY).astype(np.float32) / 255.0
            )
...
def _generate_preview_and_error(
    ...
    """生成预览和误差图"""
    try:
        pred_labs_front = solver._predict_batch(
            np.asarray(unique_recipe_indices_solved, dtype=np.int32)
        )
        ...
        recipes_back_view = np.asarray(unique_recipe_indices_print, dtype=np.int32)[
            :, ::-1
        ]
        ...
        Image.fromarray(pred_full_back).save(
            out_run_dir / "01_preview_back_predicted.png"
        )

        sign = (
            pred_full_front.astype(np.int32) - pred_full_back.astype(np.int32)
        ).mean(axis=2)
        ...
        Image.fromarray(sign_viz).save(
            out_run_dir / "01_preview_front_vs_back_sign.png"
        )

        create_difference_visualization(
            rgb_u8, pred_full_front, str(out_run_dir / "01_error_heatmap_predicted.png")
        )
        ...
        (out_run_dir / "01_error_stats_predicted.json").write_text(
