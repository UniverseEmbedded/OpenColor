    ...
    *,
    ...
    def _soft_blur_01(arr_u8: np.ndarray) -> np.ndarray:
        ...
        if _HAS_CV2:
            try:
                f01 = _to01(arr_u8)
                f2 = cv2.GaussianBlur(f01, ksize=(0, 0), sigmaX=sigma, sigmaY=sigma, borderType=cv2.BORDER_REPLICATE)
        ...
        try:
            ...
            logger.error(f"警告: PIL 高斯模糊失败，将跳过 soft_edge_sigma_px。原因: {e}")
            return _to01(arr_u8)

    t_all0 = perf_counter()
    for layer_str, files in sorted(layers.items()):
        t0 = perf_counter()
        print_ts(f"[信息] 检查层质量(精细重叠与空缺分析): {layer_str}, 文件数={len(files)}")
        ...
        for p in files:
            ...
            if m_u8.shape != (h, w):
                logger.warning(f"  警告: mask {slot_name} 尺寸不匹配 {m_u8.shape} != {(h, w)}，正在自动 resize...")
                m_u8 = np.array(Image.fromarray(m_u8).resize((w, h), Image.BILINEAR), dtype=np.uint8)
        ...
        if tol > 0:
            if not _HAS_CV2:
                logger.warning("  [警告] boundary_tolerance_px 已设置，但 cv2 不可用，将忽略边界容差")
            else:
                k = 2 * tol + 1
                kernel = np.ones((k, k), dtype=np.uint8)
                masks_for_overlap = [cv2.erode(m, kernel, iterations=1) for m in masks_u8]
        ...
        if full_mask is not None:
            ...
            if f_mask_u8 is not None and f_mask_u8.shape != (h, w):
                f_mask_u8 = np.array(Image.fromarray(f_mask_u8).resize((w, h), Image.BILINEAR), dtype=np.uint8)
        ...
        overlap_heatmap_path = mask_path / f"{layer_str}{output_suffix}_overlap_heatmap.png"
        ...
        logger.info(f"  层 {layer_str}: 重叠面积 = {overlap_area:.2f} px, 空缺面积 = {gap_area:.2f} px")
        ...
        report["layers"][layer_str] = {
            ...
            "gap_heatmap_path": str(gap_heatmap_path) if full_mask is not None else None
        }
    ...
    logger.info(f"检测完成: 总重叠面积 = {report['total_overlap_area']:.2f}, 总空缺面积 = {report['total_gap_area']:.2f}")
...
if __name__ == "__main__":
    # 更新为 oc_proto 的路径
    check_mask_overlaps(r"D:\pama1234\pfp\p-2026-01\OpenColor-02\py_module\prototypes\src\oc_proto\gen_vector\out\02_masks")
