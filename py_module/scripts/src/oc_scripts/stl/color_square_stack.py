...
def main() -> None:
    ...
    ap.add_argument("--hex", default="#DDF4C4", help="颜色十六进制 #RRGGBB 或 #RRGGBBAA")
    ...
    ap.add_argument("--first-layer-height", type=float, default=0.12, help="首层高度（毫米）")
    ap.add_argument("--view", default="bottom", choices=["top", "bottom"], help="观察面（顶面或底面）")
    ap.add_argument("--backing", default="white", choices=["white", "black"], help="前向模型使用的背板颜色")
    ap.add_argument("--phys", action="store_true", help="使用蒙特卡洛物理前向模型进行选择")
    ap.add_argument("--samples", type=int, default=80000)
    ap.add_argument("--candidates", type=int, default=64)
    ap.add_argument("--brute-force-threshold", type=int, default=60000, help="当 K^L <= 阈值时枚举所有序列")
    ap.add_argument("--opacity-weight", type=float, default=-1.0, help="不透明度项权重；<0 表示自动")
    ap.add_argument("--alpha-background", default="white", choices=["white", "black"], help="解释 RGBA 时使用的背景")
    ap.add_argument("--pattern", default="balanced", choices=["balanced", "grouped"], help="非暴力搜索时使用的模式")
    ...
    logger.info(f"pred={p.pred_rgb_srgb}  loss={p.loss:.6f}  total_thickness_mm={total_h:.3f}  view={p.view} backing={p.backing}")
