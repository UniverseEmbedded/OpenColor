...
def extrude_union_to_mesh(
    polys_mm: List[Polygon],
    thickness_mm: float,
    simplify_mm: float,
    min_area_mm2: float,
    ...
    for part in parts:
        ...
        try:
            ...
            try:
                m = trimesh.creation.extrude_polygon(
                    pp, height=thickness_mm, engine="earcut"
                )
                meshes.append(m)
            except Exception:
                continue
...
def main():
    ap = argparse.ArgumentParser(
        description="纯矢量SVG -> 4个STL（RGBW）。无栅格化。无堆叠。"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.2,
        help="曲线近似容差（毫米）（越小=越平滑，段数越多）",
    )
    ap.add_argument(
        "--simplify-mm", type=float, default=0.15, help="多边形简化容差（毫米）"
    )
    ap.add_argument(
        "--min-area-mm2",
        type=float,
        default=0.8,
        help="丢弃小于此面积的微小多边形（平方毫米）",
    )
    args = ap.parse_args()
    ...
    logger.info(
        f"比例 {scale:.6f} 毫米/单位, 物理尺寸 {args.width_mm:.2f} x {height_mm:.2f} 毫米"
    )
