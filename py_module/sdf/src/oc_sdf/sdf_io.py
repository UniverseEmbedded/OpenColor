...
def _load_svg_polygons_python(svg_path: Path) -> List[Polygon]:
    ...
    try:
        ...
        def _flush_ring(r: List[Tuple[float, float]]):
            ...
            # 如果首尾点相同，移除显式闭合点
            if len(r) >= 2 and (
                abs(r[0][0] - r[-1][0]) < 1e-9 and abs(r[0][1] - r[-1][1]) < 1e-9
            ):
                r = r[:-1]
            # 移除连续重复点
            cleaned = []
...
def rasterize_geometry_soft(
    ...
    for p in polys:
        ...
        exterior = [
            (float(x) * spx, float(height_mm - y) * spx) for x, y in pp.exterior.coords
        ]
        if len(exterior) >= 3:
        ...
        for ring in pp.interiors:
            hole = [
                (float(x) * spx, float(height_mm - y) * spx) for x, y in ring.coords
            ]
            if len(hole) >= 3:
                draw.polygon(hole, fill=0)
