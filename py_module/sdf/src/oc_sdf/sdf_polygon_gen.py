...
def generate_slot_layer_polygon(
    ...
    clip_poly = build_clip_poly(
        layer_mask, grid_scale, match_h_px, params.nozzle_width_mm, pad_px, True
    )

    layer_mask_hi, pad_hi = prepare_polygon_mask(
        layer_mask, grid_scale, pad_px, blur_radius, True, keep_float=False
    )
    ...
    rings, used_backend = mask_to_rings(
        ...
        vtracer_filter_speckle=int(getattr(sdf_params, "vtracer_filter_speckle", 4))
        * max(1, grid_scale * grid_scale // 4),
        vtracer_segment_length=int(getattr(sdf_params, "vtracer_segment_length", 4)),
        vtracer_corner_threshold=int(
            getattr(sdf_params, "vtracer_corner_threshold", 60)
        ),
    ...
    temp_polys = rings_to_polys(
        rings, match_h_px, grid_scale, params.nozzle_width_mm, pad_hi
    )
    if not temp_polys:
        return None, clip_poly, used_backend
    ...
    if (
        sdf_params.enable_clip_intersection
        and clip_poly is not None
        and not clip_poly.is_empty
    ):
        ...
        try:
            ...
            io_save_svg(
                clip_poly, gap_dir / f"{prefix}_clip_geom.svg", width_mm, height_mm
            )
        except Exception as e:
        ...
        try:
            ...
            slot_layer_poly = slot_layer_poly.buffer(0).intersection(
                clip_poly.buffer(0)
            )
    ...
    # 填充缝隙
    if (
        sdf_params.enable_gap_filling
        and sdf_params.fill_gap_mm
        and sdf_params.fill_gap_mm > 0
    ):
        try:
    ...
    if sdf_params.enable_mutual_exclusion and not geom_is_empty(layer_occupied):
        try:
            ...
            slot_layer_poly = slot_layer_poly.buffer(0).difference(
                layer_occupied.buffer(0)
            )
    ...
    if not geom_is_empty(slot_layer_poly):
        try:
            save_svg(
                slot_layer_poly,
                poly_dir / f"{prefix}_poly_final.svg",
                width_mm,
                height_mm,
            )
            poly_mask = rasterize_geometry(
                slot_layer_poly, match_w_px, match_h_px, height_mm, px_per_mm
            )
            if poly_mask is not None:
                Image.fromarray(poly_mask.astype(np.uint8) * 255, mode="L").save(
