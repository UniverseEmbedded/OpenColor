...
class NumpyFourFluxModel(OpticalModel):
    ...
    def _sigmoid(self, x):
        ...
        return np.float32(1.0) / (np.float32(1.0) + np.exp(-np.clip(x, np.float32(-20.0), np.float32(20.0))))
    ...
    def _params_to_four_flux(self, mu_a_raw, mu_s_raw, g_raw):
        ...
        k = np.exp(np.clip(mu_a_raw, np.float32(-10.0), np.float32(10.0))).astype(np.float32)
        s = np.exp(np.clip(mu_s_raw, np.float32(-10.0), np.float32(10.0))).astype(np.float32)
        ...
        rd = (np.float32(0.5) * omega_d * (np.float32(1.0) - np.exp(-(np.float32(2.0) * tau_d)))).astype(np.float32)
        td = (np.exp(-tau_d).astype(np.float32) + rd).astype(np.float32)
        
        for _ in range(14):
            denom = (np.float32(1.0) - rd * rd).astype(np.float32)
            denom = np.where(np.abs(denom) < np.float32(1e-6), np.float32(1e-6), denom).astype(np.float32)
            td_new = (td * td / denom).astype(np.float32)
            rd_new = (rd + (td * rd * td) / denom).astype(np.float32)
            rd, td = rd_new, td_new

        return rc.astype(np.float32), tc.astype(np.float32), rd.astype(np.float32), td.astype(np.float32)
    ...
    def _apply_backing_four_flux(self, Rd, Td, Rc, Tc, backing):
        ...
        denom_d = np.where(np.abs(denom_d) < np.float32(1e-6), np.float32(1e-6), denom_d).astype(np.float32)
    ...
    def _apply_saunderson(self, Ri, k1, k2):
        ...
        if float(k1_f) == 0.0 and float(k2_f) == 0.0:
            return Ri
        return (k1_f + (np.float32(1.0) - k1_f) * (np.float32(1.0) - k2_f) * Ri / (np.float32(1.0) - k2_f * Ri + np.float32(1e-9))).astype(np.float32)

    def predict_rgb(self, sequences, material_keys, params, backing=0.98, k1=0.0, k2=0.0):
        ...
        for layer_idx in range(max_len):
            ...
            n_Rc, n_Tc, n_Rd, n_Td = self._stack_four_flux(
                R_c_stack[valid_mask], T_c_stack[valid_mask], R_d_stack[valid_mask], T_d_stack[valid_mask],
                Rc[valid_idx], Tc[valid_idx], Rd[valid_idx], Td[valid_idx]
            )
            R_c_stack[valid_mask], T_c_stack[valid_mask], R_d_stack[valid_mask], T_d_stack[valid_mask] = n_Rc, n_Tc, n_Rd, n_Td
                
        rgb_lin_internal = self._apply_backing_four_flux(R_d_stack, T_d_stack, R_c_stack, T_c_stack, backing)
        rgb_lin_measured = self._apply_saunderson(rgb_lin_internal, k1, k2)
        return np.clip(linear01_to_srgb01(rgb_lin_measured), 0.0, 1.0).astype(np.float32)
    ...
    def fit(self, sequences, y_lab, material_keys, initial_params, **kwargs):
        ...
        def _loss(x):
            ...
            pred_rgb = self.predict_rgb(sequences, material_keys, p, backing=backing_val, k1=k1, k2=k2)
            pred_lab = rgb01_to_lab(pred_rgb)
            sample_loss = np.mean(np.square(pred_lab - y_lab), axis=1)
            data_loss = float(np.sum(sample_weight * sample_loss) / (np.sum(sample_weight) + 1e-9))
            reg_loss = reg_val * (np.sum(np.square(mu_a)) + np.sum(np.square(mu_s)) + np.sum(np.square(g)))
            return data_loss + reg_loss

        x0_list = [initial_params.mu_a.flatten(), initial_params.mu_s.flatten(), initial_params.g.flatten()]
        if optimize_k:
        ...
        if optimize_k:
            ...
            logger.info(f"[NumpyModel] 拟合得到的 Saunderson 参数: k1={fitted_k1:.4f}, k2={fitted_k2:.4f}")
...
class VulkanFourFluxModel(OpticalModel):
    ...
    def predict_rgb(self, sequences, material_keys, params, backing=0.98, k1=0.04, k2=0.6):
        key_to_idx = {k.upper(): i for i, k in enumerate(material_keys)}
        n_seq = len(sequences)
        if n_seq == 0:
            return np.zeros((0, 3), dtype=np.float32)
        ...
        try:
            ...
            out_srgb_arr = np.asarray(out_srgb_raw, dtype=np.float32).reshape((n_seq, 3))
            out_lin = srgb_to_linear01(out_srgb_arr)
            out_lin_corr = self._apply_saunderson(out_lin, k1, k2)
            return np.clip(linear01_to_srgb01(out_lin_corr), 0.0, 1.0).astype(np.float32)
    ...
    def fit(self, sequences, y_lab, material_keys, initial_params, **kwargs):
        ...
        if optimize_k:
            fitted_k1 = float(np.clip(res.x[n_mat*9], 0.0, 0.2))
            fitted_k2 = float(np.clip(res.x[n_mat*9+1], 0.0, 1.0))
            logger.info(f"[VulkanModel] 拟合得到的 Saunderson 参数: k1={fitted_k1:.4f}, k2={fitted_k2:.4f}")
