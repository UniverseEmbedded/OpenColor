...
from oc_xgb.optical_model import OpticalParams as RawOpticalParams, NumpyFourFluxModel, VulkanFourFluxModel
...
class OpticalParams:
    ...
    @classmethod
    def from_raw(cls, raw: RawOpticalParams, material_keys: list[str], n_layers: int, k1: float = 0.0, k2: float = 0.0, backing: float = 0.98) -> OpticalParams:
        return cls(
...
def get_optical_model(use_vulkan: bool = True, model_type: str = "tmm"):
    ...
    import warnings
    if model_type == "tmm":
        warnings.warn("TMM模型已废弃，请使用RTS模型代替", DeprecationWarning, stacklevel=2)
        raise ValueError(f"已废弃的光学模型类型: {model_type}")
    elif model_type == "four_flux":
        warnings.warn("FourFlux模型已废弃，请使用RTS模型代替", DeprecationWarning, stacklevel=2)
        # 使用FourFlux模型
        if use_vulkan:
...
def predict_ad_rgb01(
    ...
    return model.predict_rgb(sequences, material_keys, params.to_raw(), backing=b, k1=k1_v, k2=k2_v)
...
def fit_optical_params(
    ...
    for i, mat in enumerate(material_keys):
        mat_u = str(mat).upper()
        mask = np.array([all(str(name).upper() == mat_u for name in seq) for seq in sequences], dtype=bool)
    ...
    if cfg.optical_model == "tmm":
        ...
        if n_total <= 0:
            ...
            fitted_k1 = cfg.k1
            fitted_k2 = cfg.k2
            return OpticalParams.from_raw(raw_fitted, material_keys, cfg.n_layers, k1=fitted_k1, k2=fitted_k2, backing=cfg.backing)

        # 阶段1：全局尺度调整
        is_pure = np.array(
            [
                (len(seq) > 0) and all(str(x).upper() == str(seq[0]).upper() for x in seq)
        ...
        def _objective_global(x: np.ndarray) -> float:
            ...
            params_adj = RawOpticalParams(mu_a=mu_a_adj, mu_s=mu_s_adj, g=initial_params.g)
        ...
        try:
            ...
            logger.info("[calib_color_model_fit] TMM 全局尺度拟合完成: "
        ...
        for i, mat in enumerate(material_keys):
            ...
            def _objective_material(x: np.ndarray) -> float:
                ...
                pred_rgb = model_cpu.predict_rgb(seq_mat, material_keys, p, backing=cfg.backing, k1=cfg.k1, k2=cfg.k2)
        ...
        raw_fitted = RawOpticalParams(mu_a=final_mu_a, mu_s=final_mu_s, g=adjusted_params.g)
        fitted_k1 = cfg.k1
        fitted_k2 = cfg.k2
        ...
        raw_fitted, fitted_k1, fitted_k2 = model.fit(
            sequences, y_lab, material_keys, initial_params, 
...
def _fit_gpr(X: np.ndarray, y: np.ndarray, cfg: PhysGPRConfig) -> GPRModel:
    ...
    signal_var = float(cfg.gpr_signal) if cfg.gpr_signal > 0 else float(np.var(ys) + 1e-6)
    ...
    for _ in range(6):
        try:
            L = np.linalg.cholesky(K + (noise + jitter) * np.eye(K.shape[0], dtype=np.float32))
            break
        except Exception:
            jitter *= 10.0
    else:
...
def train_phys_gpr(
    ...
    sequences = build_layer_sequences(recipes, material_keys, cfg.n_layers, layer_names_order=cfg.layer_names_order)
    ...
    if conflict_groups:
        ...
        logger.warning(f"[calib_color_model_fit] 警告: 发现特征冲突！{len(X_features)} 个样本中存在 {len(conflict_groups)} 组冲突，共涉及 {total_conflicting_samples} 个格子。")
        
        if metadata:
            logger.info("[calib_color_model_fit] --- 前 10 组冲突详情 ---")
            sorted_groups = sorted(conflict_groups.items(), key=lambda x: len(x[1]), reverse=True)
            for i, (feat, indices) in enumerate(sorted_groups[:10]):
                ...
                clean_recipe = {k: v for k, v in rep_recipe.items() if not k.startswith("_")}
                logger.info(f"  冲突组 {i+1} ({len(indices)}个重复): {', '.join(samples)} | 配方内容: {clean_recipe}")
    ...
    logger.info(f"[calib_color_model_fit] 训练集平均 DeltaE: {np.mean(err):.4f}, 最大 DeltaE: {np.max(err):.4f}")

    return PhysGPRModel(optical=optical, gpr_L=gpr_L, gpr_a=gpr_a, gpr_b=gpr_b, feature_names=feature_names)
...
def predict_phys_gpr_lab(
    ...
    def predict_gpr(gpr: GPRModel, X: np.ndarray):
        ...
        # 计算核矩阵
        # K(X, X_train)
        sq_dist = np.sum(X_norm**2, axis=1).reshape(-1, 1) + np.sum(gpr.X_train**2, axis=1) - 2 * np.dot(X_norm, gpr.X_train.T)
