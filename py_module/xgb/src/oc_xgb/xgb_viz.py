...
def render_board_rgb(cells, rgb_by_rc: Dict[Tuple[int,int], Optional[Tuple[int,int,int]]], default=(0,0,0), cell_px: int = 64, border_px: int = 2):
    """渲染RGB色盘图像
...
def render_mask(cells, mask_by_rc: Dict[Tuple[int,int], bool], cell_px: int = 64, border_px: int = 2):
    """渲染掩码图像
...
def render_error_heatmap(cells, err_by_rc: Dict[Tuple[int,int], float], vmax: float, cell_px: int = 64, border_px: int = 2):
    """渲染误差热力图
    ...
    # 添加标注
    cv2.putText(canvas, f"{vmax:.1f}", (W + 4, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA)
    cv2.putText(canvas, "0.0", (W + 4, H - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA)
    cv2.putText(canvas, "err", (W + 8, H//2), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA)
    return canvas
