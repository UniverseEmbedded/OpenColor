...
# Pure vector-region pipeline: SVG -> 4 STL solids (R,G,B,W).
# Improvements vs your original:
# - Supports common SVG transform attribute: translate/scale/rotate/matrix
# - Strict palette mode (default): only accept exact RGBW fill colors
# - Optional nearest-palette classification (off by default)
# - More explicit warnings for unsupported features
...
def classify_rgb_to_rgbw_strict(rgb255: Tuple[int, int, int], tol: int = 0) -> Optional[str]:
    # tol=0 means exact match; tol>0 allows small difference (e.g. if exporter perturbs)
...
def parse_transform_attr(s: str) -> np.ndarray:
    """
    Parse a minimal subset of SVG transform list into a 3x3 affine matrix.
    Supports: matrix(a,b,c,d,e,f), translate(tx,ty), scale(sx,sy), rotate(angle[,cx,cy])
    Order in SVG is left-to-right application; for column vectors we right-multiply accordingly.
    We'll build a matrix M such that p' = M * p.
    """
    if not s:
    ...
    # tokenize transform functions
    # e.g. "translate(10,20) rotate(30) scale(2)"
...
def approx_closed_rings_from_path(path: Path, tol_units: float, M: np.ndarray) -> List[List[Tuple[float, float]]]:
    ...
    for sub in path.continuous_subpaths():
        ...
        if abs(sub.start - sub.end) > 1e-9:
            # only closed shapes supported for "fill regions"
            continue
...
def build_polygons_from_rings(rings: List[List[Tuple[float, float]]]) -> List[Polygon]:
    ...
    for r in ring_infos:
        ...
        if container_i is None:
            ...
            # opposite orientation => hole for that outer, else treat as nested outer
...
def transform_svg_units_to_mm(poly: Polygon, vb_minx: float, vb_miny: float, vb_h: float, scale_mm_per_unit: float) -> Polygon:
    # Map SVG coords -> mm and flip Y so +Y is "up" in print space
...
def extrude_polygon_to_mesh(geom, height_mm: float):
    # trimesh.creation.extrude_polygon supports Polygon; for MultiPolygon do union outside
    if geom.is_empty:
        return None
...
def main():
    ap = argparse.ArgumentParser(description="SVG -> 4 STL (R,G,B,W) by vector regions (no stacking).")
    ap.add_argument("svg", help="input SVG")
    ap.add_argument("--outdir", default="out_svg4stl", help="output directory")
    ap.add_argument("--name", default=None, help="base name")
    ap.add_argument("--width-mm", type=float, default=80.0, help="target width in mm")
    ap.add_argument("--thickness-mm", type=float, default=0.8, help="extrusion thickness for all regions")
    ap.add_argument("--tol-mm", type=float, default=0.2, help="sampling tolerance in mm-equivalent (controls polygon complexity)")
    ap.add_argument("--simplify-mm", type=float, default=0.15, help="shapely simplify tolerance in mm")
    ap.add_argument("--min-area-mm2", type=float, default=0.2, help="drop tiny islands below area in mm^2")
    ap.add_argument("--strict-palette", action="store_true", help="only accept exact RGBW fill colors (recommended for testing)")
    ap.add_argument("--palette-tol", type=int, default=0, help="strict match tolerance in RGB 0..255 per channel")
    ap.add_argument("--nearest-palette", action="store_true", help="classify arbitrary fill to nearest RGBW (use with caution)")
    ap.add_argument("--export-metadata", action="store_true")
    args = ap.parse_args()

    if args.nearest_palette and args.strict_palette:
        print("Error: choose either --strict-palette or --nearest-palette, not both.")
        raise SystemExit(2)
    ...
    # sampling tolerance in SVG units
    ...
    for path, a, M, fill_set, fill_value in paths:
        ...
        if args.strict_palette:
            ...
            # default behavior: strict, but with a helpful warning
            key = classify_rgb_to_rgbw_strict(fill, tol=args.palette_tol)
            if key is None:
                if not warned_unsupported:
                    print("Warning: non-RGBW fills found. Use --nearest-palette to auto-classify or --strict-palette to skip them.")
    ...
    for key in ["R", "G", "B", "W"]:
        ...
        if mesh is None:
            print(f"{key}: extrusion failed (check shapely validity / earcut install)")
            continue
    ...
    if skipped:
        print(f"Skipped {skipped} paths (no fill / non-palette fill / non-closed).")
    print("Note: clipPath, gradients, text, strokes are not expanded. Export as paths with solid fills for best results.")
