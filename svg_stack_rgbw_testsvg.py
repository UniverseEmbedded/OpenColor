...
# SVG -> stacked RGBW planar content exported as 4 STL.
#
# This script is intended for "bottom-side color" experiments (image appears on the bed-contact face),
# so the default view is "bottom".
#
# Example:
...
def parse_style_value(style: str, key: str) -> Optional[str]:
    if not style:
        return None
    m = re.search(rf"{re.escape(key)}\s*:\s*([^;]+)", style, re.IGNORECASE)
...
def parse_transform_attr(s: str) -> np.ndarray:
    ...
    Order is left-to-right application.
    """
    if not s:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="SVG -> stacked RGBW planar content as 4 STL")
    ap.add_argument("--svg", default="test.svg", help="input svg")
    ap.add_argument("--outdir", default="out_svg_stack", help="output directory")
    ap.add_argument("--name", default="testsvg_stack", help="base name")
    ap.add_argument("--mode", default="rgbw")
    ap.add_argument("--width-mm", type=float, default=90.0, help="target width in mm")
    ap.add_argument("--first-layer-height", type=float, default=0.12)
    ap.add_argument("--layer-height", type=float, default=0.08)
    ap.add_argument("--layers", type=int, default=5)
    ap.add_argument("--pattern", default="balanced", choices=["balanced", "grouped"], help="used only in non-bruteforce uniform search")

    # Default to bottom-side viewing (bed-contact face)
    ...
    ap.add_argument("--opacity-weight", type=float, default=-1.0, help="<0 means auto: only enable when alpha != 255")
    ...
    ap.add_argument("--alpha-background", default="white", choices=["white", "black"], help="for RGBA alpha compositing target")
    ...
    print(f"view={args.view} backing={args.backing} alpha_background={args.alpha_background}")
    print(f"layers={args.layers} heights={layer_heights} total={total_h:.3f}mm")
    ...
    # Collect shapes in draw order: each shape -> (poly_mm, rgba)
    ...
    if not shapes:
        raise SystemExit("No filled closed shapes found in SVG.")

    # Cache per-RGBA plans
    plan_cache: Dict[RGBA, planner.PlanResult] = {}

    # For each layer, maintain occupied area to prevent overlaps (painter order)
    occupied_by_layer: List[Optional[Polygon]] = [None for _ in range(int(args.layers))]

    # Buckets: token -> list of (poly, layer_index)
    ...
    for poly, rgba in reversed(shapes):
        if rgba not in plan_cache:
            ...
            p0 = plan_cache[rgba]
            print(f"rgba={rgba} seq={'-'.join(p0.sequence)} pred={p0.pred_rgb_srgb} loss={p0.loss:.6f}")

        seq = plan_cache[rgba].sequence

        # painter algorithm per layer: later shapes override earlier
    ...
    # Build meshes per material by unioning per layer then extruding at correct z
    z_offsets = []
    z = 0.0
