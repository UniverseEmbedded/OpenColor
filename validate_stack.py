...
class CaseResult:
    ...
    seq: List[str]
    heights: List[float]
    fast_pred: Tuple[int, int, int]
    phys_pred: Tuple[int, int, int]
    loss_fast_to_target: float
    loss_phys_to_target: float
    loss_phys_to_fast: float


def main(argv: Optional[Sequence[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Validate fast stack predictions against physical forward model.")
    ...
    if args.seq is not None or args.heights is not None:
        ...
        print(f"samples={samples} mode={mode_key} view={args.view} backing={args.backing}")
        print(f"seq={'-'.join(seq)}")
        print(f"fast_pred={fast_srgb} phys_pred={phys_srgb} loss_phys_to_fast={loss_phys_to_fast:.6f}")
    ...
    print(f"cases={len(results)} samples={samples} mode={mode_key} view={args.view} backing={args.backing}")
    print(f"avg_loss_phys_to_fast={avg_df:.6f} avg_loss_phys_to_target={avg_dt:.6f}")
    print(
