...
test.describe('校准色盘生成测试 - Tauri简单模式', () => {
  test('生成1张8色校准色盘', async ({ page }) => {
    console.log('开始测试：生成1张8色校准色盘 (Tauri简单模式)');
    ...
    // 截图：页面加载后
    await page.screenshot({ path: 'test-results/tauri-simple-01-board-gen-page.png' });
    ...
    if (options.length <= 1) {
      ...
      await page.screenshot({ path: 'test-results/tauri-simple-02-no-profiles.png' });
    ...
    await delay(1500);
    await page.screenshot({ path: 'test-results/tauri-simple-02-profile-selected.png' });
    ...
    // 截图：生成完成后
    await page.screenshot({ path: 'test-results/tauri-simple-03-generation-complete.png', fullPage: true });
    ...
    if (errorMessage) {
      ...
      await page.screenshot({ path: 'test-results/tauri-simple-04-error.png', fullPage: true });
    ...
    if (cards.length === 0) {
      ...
      await page.screenshot({ path: 'test-results/tauri-simple-04-no-cards.png', fullPage: true });
    ...
    await cards[0].click();
    await delay(1500);
    await page.screenshot({ path: 'test-results/tauri-simple-05-card-selected.png', fullPage: true });
    ...
    // 最终截图
    await page.screenshot({ path: 'test-results/tauri-simple-06-final.png', fullPage: true });
